...
typedef enum __hypo_type {hint, hreal, hchar, halfa, hbool} hypo_type;
...
void codegenExpr(unsigned int prod)
{
    if (loadfile != NULL)
    {
        //TODO lookup production, perform codegen operation
        //fwrite(&loadrec, sizeof(loadrec), 1, loadfile);
    }
}
